...
> = RequestDocument | TypedDocumentNode<TData, TVariables>;
...
export type GraphqlDocumentRoot<TDocument> =
    TDocument extends GraphqlDocumentTypeDecoration<
        infer TRoot,
        infer _TVariables extends GraphqlVariables
    >
        ? TRoot
        : unknown;
export type GraphqlDocumentVariables<TDocument> =
    TDocument extends GraphqlDocumentTypeDecoration<
        unknown,
        infer TVariables extends GraphqlVariables
    >
        ? TVariables
        : GraphqlVariables;
...
type GraphqlArrayElementValue<T> =
    GraphqlNonNullableValue<T> extends readonly (infer Item)[]
        ? GraphqlSelectionValue<Item>
        : never;
type GraphqlObjectKey<T extends object> = Exclude<keyof T & string, GraphqlMetaKey>;

type GraphqlSingleKey<
    T extends object,
    Key extends GraphqlObjectKey<T> = GraphqlObjectKey<T>,
> = Key extends Key ? (Exclude<GraphqlObjectKey<T>, Key> extends never ? Key : never) : never;
...
>(
    ...
    TRoot,
    ParseKey extends string ? ParseKey : GraphqlInferredParseKey<TRoot>,
    TVariables,
    TKey
>;

type DefineGraphql = {
    <TRoot, TVariables extends GraphqlVariables = GraphqlVariables>(): DefineGraphqlFactory<
        TRoot,
        TVariables
    >;
    <
    ...
    >(
        ...
        ParseKey extends string
            ? ParseKey
            : GraphqlInferredParseKey<GraphqlDocumentRoot<TDocument>>,
